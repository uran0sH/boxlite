...
///
/// This validates platform prerequisites (KVM on Linux, Hypervisor.framework on macOS)
/// before attempting to create VMs. Follows "Validate Early" principle - fail fast
/// with clear diagnostics instead of obscure errors from libkrun later.
///
/// # Errors
///
/// Returns `BoxliteError::Unsupported` with diagnostic information if:
/// - Linux: `/dev/kvm` doesn't exist or isn't accessible
/// - macOS: Wrong architecture (only ARM64 supported)
///
/// # Examples
///
/// ```no_run
...
fn check_macos_hypervisor() -> BoxliteResult<VirtualizationSupport> {
    ...
    {
        // Use sysctl to check Hypervisor.framework support
        match check_sysctl_hv_support() {
            Ok(true) => Ok(VirtualizationSupport {
                reason: "Hypervisor.framework is available (Apple Silicon)".to_string(),
            }),
            Ok(false) => Err(BoxliteError::Unsupported(
                "Hypervisor.framework is not available\n\n\
                 Suggestions:\n\
...
/// Check Hypervisor.framework support using sysctl.
///
/// Returns:
/// - Ok(true): Hypervisor.framework is available
/// - Ok(false): Hypervisor.framework is not available
/// - Err(e): Failed to check (sysctl error)
#[cfg(target_os = "macos")]
fn check_sysctl_hv_support() -> Result<bool, std::io::Error> {
    use std::process::Command;

    let output = Command::new("sysctl").arg("kern.hv_support").output()?;
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[cfg(target_arch = "aarch64")]
    fn test_check_sysctl_hv_support() {
        // Test that sysctl check runs and returns a result
        match check_sysctl_hv_support() {
            Ok(supported) => {
            ...
            Err(e) => {
                // sysctl command failed (shouldn't happen on macOS)
                panic!("sysctl kern.hv_support failed: {}", e);
            }
        }
    }
}
