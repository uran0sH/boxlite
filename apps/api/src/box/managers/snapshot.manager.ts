...
export class SnapshotManager implements TrackableJobExecutions, OnApplicationShutdown {
  ...
  // Pulls stopped boxes' backup snapshots to another runner to prepare for reassignment during draining
      ...
      await Promise.allSettled(
        drainingRunners.map(async (runner) => {
          try {
            ...
            this.logger.debug(
              `Found ${boxes.length} eligible boxes on draining runner ${runner.id} for snapshot migration`,
            )

            await Promise.allSettled(
              boxes.map(async (box) => {
                const boxLockKey = `draining-runner-snapshot-migration:${box.id}`
                const hasBoxLock = await this.redisLockProvider.lock(boxLockKey, 3600)
                if (!hasBoxLock) {
                  return
                }
                ...
                try {
                  ...
                  const existingEntry = await this.runnerService.getSnapshotRunner(targetRunner.id, box.backupSnapshot)
                  if (existingEntry) {
                    if (existingEntry.state === SnapshotRunnerState.ERROR) {
                      // Clean up the failed entry so we can retry
                      this.logger.warn(
                        `Removing ERROR snapshot runner entry ${existingEntry.id} for runner ${targetRunner.id} and snapshot ${box.backupSnapshot} to allow retry`,
                      )
                      await this.snapshotRunnerRepository.delete(existingEntry.id)
                    } else {
                      this.logger.debug(
                        `Snapshot runner entry already exists for runner ${targetRunner.id} and snapshot ${box.backupSnapshot} (state: ${existingEntry.state})`,
                      )
                  ...
                  const registry = box.backupRegistryId
                    ? await this.dockerRegistryService.findOne(box.backupRegistryId)
                  ...
                  if (!registry) {
                    this.logger.warn(`No registry found for backup snapshot ${box.backupSnapshot} of box ${box.id}`)
                  ...
                  await this.pullSnapshotRunner(targetRunner, box.backupSnapshot, registry)

                  this.logger.log(
                    `Created snapshot runner entry for box ${box.id} backup ${box.backupSnapshot} on runner ${targetRunner.id} (migrating from draining runner ${runner.id})`,
                  )
                  await this.redisLockProvider.unlock(boxLockKey)
                } catch (e) {
                  if (e instanceof BadRequestError && e.message === 'No available runners') {
                    this.logger.warn(
                      `No available runners found in region ${box.region} for box ${box.id} snapshot migration`,
                    )
                  } else {
                    this.logger.error(`Error migrating snapshot for box ${box.id}`, e)
                  }
                  await this.redisLockProvider.unlock(boxLockKey)
                }
              }),
